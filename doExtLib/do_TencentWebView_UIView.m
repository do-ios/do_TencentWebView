...
#import "do_TencentWebView_UIView.h"
...
#import "doTencentWebViewProgressView.h"
#import "do_TencentWebView.h"

float doTencentInitialProgressValue = 0.6f;
const float doTencentFinalProgressValue = 0.9f;
const float doTencent_urltimeout = 60;

@interface do_TencentWebView_UIView()<doEGORefreshTableDelegate,UIWebViewDelegate,UIScrollViewDelegate>
...
@implementation do_TencentWebView_UIView
{
    ...
    //progress view
    doTencentWebViewProgressView *_progressView;
...
{
    model = (do_TencentWebView_UIModel *)_doUIModule;
...
- (void) OnRedraw
{
    ...
    if (!_progressView && _isShowLoadingProgress) {
        _progressView = [[doTencentWebViewProgressView alloc] initWithFrame:barFrame];
...
- (void)startProgress
{
    if (_progress < doTencentInitialProgressValue) {
        [self setProgress:doTencentInitialProgressValue];
    }
}
...
- (void)incrementProgress
{
    ...
    float maxProgress = doTencentFinalProgressValue;
...
{
    for (int i=0; i<6; i++) {
        doTencentInitialProgressValue = (1 + (arc4random() % 6))/10.0;
    }
...
- (void)navigate:(NSString *)_fullUrl
{
    ...
    [_webView loadRequest:[NSURLRequest requestWithURL:loadUrl cachePolicy:policy timeoutInterval:doTencent_urltimeout]];
}
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    do_TencentWebView *tencentWebView = [do_TencentWebView new];
    tencentWebView.eventCenter = model.EventCenter;
    //    IOS *ios=[IOS new];
    self.jsContext[@"do_TencentWebView"]=tencentWebView;
...
- (void)setCookie:(NSArray *)_parms
{
    ...
    [cookieProperties setObject:[cookieHost path] forKey:NSHTTPCookiePath];
...
- (void)eval:(NSArray *)parms
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        result = [[_webView stringByEvaluatingJavaScriptFromString:code] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
